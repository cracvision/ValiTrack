...
-- BUG-2: Filter out tasks and sign-offs from terminal review cases

-- Update get_user_intray_items: add rc.status filter to Sections 1 & 2
...
BEGIN
  ...
  WHERE rt.is_deleted = false
    ...
    AND rc.status NOT IN ('approved', 'rejected', 'cancelled')
    AND (v_target_user_id IS NULL OR rt.assigned_to = v_target_user_id)

  UNION ALL

  -- SECTION 2: REVIEW SIGN-OFFS
  SELECT
    'review_signoff'::TEXT,
    rs.id,
  ...
  -- SECTION 3: PROFILE SIGN-OFFS (unchanged)
  SELECT
  ...
  -- SECTION 4: SO action — advance to plan_approval
  SELECT
    'action'::TEXT,
    rc.id,
    rc.system_owner_id,
  ...
  -- SECTION 5: QA action — approve plan
  SELECT
    'action'::TEXT,
    rc.id,
    rc.qa_id,
  ...
  -- SECTION 6: SO action — start execution
  SELECT
    'action'::TEXT,
    rc.id,
    rc.system_owner_id,
  ...
  -- SECTION 8: QA action — final approval
  SELECT
    'action'::TEXT,
    rc.id,
    rc.qa_id,
...
-- Update get_user_intray_count: add rc.status filter to tasks and review signoffs
...
BEGIN
  ...
  -- Tasks: exclude terminal review cases
  ...
  -- Sign-offs: exclude terminal review cases for review_signoffs
  SELECT (
  ...
  -- Actions: unchanged (already filter by specific active statuses)
  SELECT (
